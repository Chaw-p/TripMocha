...
from .models import db, TripMain , TripDetail, TripMapping
...
@info_bp.route("/location/<trip_no>/<day_no>", methods = ["PUT"])
def update_trip_detail(trip_no,day_no): 
  data = request.get_json().get("data");

  if not data:
    return jsonify({"message": "전송된 데이터가 없습니다."}), 400
  
  # None 체크
  # trip 여부 체크
  trip = TripMain.query.filter_by(trip_no=trip_no).first()
  if not trip:
    return jsonify({"message": f"ID가 {trip_no}인 여행을 찾을 수 없습니다."}), 404

  # new_trip_detail로 보낸것을 가져온다.
  required_fields = ["detail_name","trip_detail", "address", "latitude", "longitude"]
  missing_fields = []
  ...
  trip_detail = data.get("trip_detail")
  ...
  try:
    ...
    #순서 값 넣기, 만약 중간에 지역추가를 할시 이 부분 변경
    max_order = db.session.query(func.max(TripMapping.visit_order))\
      .filter_by(trip_no=trip_no, day_sequence=day_no)\
      .scalar()
